...
function generateForestMetadata(): Row {
    const accupiedTiles = new Set<number>()
    const trees = Array.from({ length: 4 }, () => {
        ...
        while (accupiedTiles.has(tileIndex))
        accupiedTiles.add(tileIndex)
...
function generateCarLaneMetadata(): Row {
  ...
  export { generateCarLaneMetadata };
...
export { generateTruckLaneMetadata };
